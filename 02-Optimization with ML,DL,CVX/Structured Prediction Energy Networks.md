...
이 부록은 SPEN inference의 실제 수렴 패턴을 그림으로 보여준다. 대부분의 예시는 빠르게 수렴하지만 일부 느린 샘플이 전체 계산 시간에 영향을 미친다. 따라서 모든 샘플의 완전 수렴을 기다리기보다 일정 비율 수렴 시 조기 종료하는 것이 효율적이다. 또한 loose한 종료 조건을 사용해도 성능 손실 없이 속도를 크게 개선할 수 있다. 즉, SPEN의 느림은 본질이 아니라 inference 전략으로 완화 가능한 문제임을 보여준다.  

#### A.2. SPEN Architecture for Multi-Label Classification  

논문 속 그림은 SPEN을 energy-based model로 이해하게 만드는 핵심 구조를 보여준다. 입력 $x$는 feature network를 통해 $F(x)$로 변환되고 출력 $\bar y$는 energy 계산에 직접 참여한다. local energy와 global energy가 결합되어 최종 energy score를 구성한다. prediction은 이 energy를 최소화하도록 $\bar y$를 반복적으로 업데이트하는 과정이다. 즉, SPEN은 단순한 mapping이 아니라 출력 공간 위의 최적화 문제로 해석된다.  

#### A.3. Deep Mean Field Predictor  

이 절은 baseline인 DMF를 CRF 기반 수식으로 정리한다. fully-connected pairwise CRF를 mean-field로 근사하면 update는 sigmoid 형태로 정리된다. 이를 벡터화하면 $\bar y \leftarrow \sigma(A\bar y - D + C)$의 recurrent update가 된다. 즉, DMF는 CRF inference를 고정 반복 구조로 펼친 unrolled network이다. 이로써 SPEN과 비교되는 DMF의 구조적 의미가 명확해진다.  

#### A.4. Details for Improving Efficiency and Accuracy of SPENs  

이 절은 SPEN을 실제로 효율적으로 사용하는 방법을 제시한다. prediction 시 momentum을 사용하면 수렴 속도를 크게 향상시킬 수 있다. soft prediction이 중요할 경우 entropy term을 추가하여 출력을 안정화할 수 있다.  
inference에서는 parameter gradient를 생략해 계산 비용을 줄일 수 있다. 또한 local → global → joint fine-tuning 순의 학습 전략이 효과적이다.  

#### A.5. Hyperparameters  

이 절은 실험에 사용된 주요 하이퍼파라미터를 정리한다. prediction은 momentum 0.95, learning rate 0.1, decay 없음의 gradient descent를 사용한다. 종료 기준은 objective 상대 변화 또는 iterate의 $l_\infty$ 변화량이다. training은 momentum 0.9의 SGD, tuned learning rate/decay, $L_2$ regularization을 사용한다. 이는 SPEN이 반복 최적화 기반 모델로서 하이퍼파라미터에 민감함을 보여준다.  
