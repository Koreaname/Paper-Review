...
SPEN의 특성 중 feature network $$F(x) = g(A_2 g(A_1 x))$$는 2-layer MLP로 정의된다. 여기서 $g(\cdot)$는 좌표별 비선형 함수이며, 실험에서는 위치에 따라 sigmoid, ReLU, HardTanh 등을 다르게 사용할 수 있다. 에너지 네트워크는 크게 local energy와 global energy의 합으로 구성된다.
...
이 절은 SPEN 예측의 실제 수렴 양상을 그림으로 보완한다. 핵심은 대부분의 샘플은 빨리 수렴하지만, 소수의 느린 샘플이 batch 전체 시간을 끌어올린다는 점이다. 또한 일정 비율의 샘플만 수렴해도 종료하거나 tolerance를 완화하면 정확도 손실을 크게 늘리지 않고도 속도를 높일 수 있다. 이에 따라 Appendix는 본문 7.4의 speed-accuracy trade-off를 시각적으로 확인해 주는 역할을 한다.

#### A.2. SPEN Architecture for Multi-Label Classification
이 절은 본문 3절의 SPEN 구조를 계산 그래프로 한 번에 정리한다. 입력 $x$로부터 $F(x)$를 만들고, 후보 출력 $\bar y$는 local energy와 global energy를 거쳐 최종 energy로 합쳐진다. 예측은 이 scalar energy를 낮추는 방향으로 $\bar y$를 반복적으로 갱신하는 과정이다. 따라서 SPEN이 단순한 feed-forward predictor가 아니라 energy landscape를 학습하는 모델임을 다시 보여준다.

#### A.3. Deep Mean Field Predictor
이 절은 비교 baseline인 DMF를 fully-connected pairwise CRF의 mean-field update에서 직접 유도한다. 확률모형을 $y^\top A y + C^\top y$ 형태로 정리한 뒤, mean-field update를 sigmoid 기반의 벡터화 반복식으로 바꾼다. 결과적으로 CRF 추론을 반복 신경계산처럼 구현한 baseline이 되며, SPEN과의 비교 기준을 제공한다. 이는 그래프 모델을 unroll한 방식과 에너지 자체를 학습한 방식의 차이를 보여주는 부록이다.

#### A.4. Details for Improving Efficiency and Accuracy of SPENs
이 절은 SPEN을 실제로 잘 동작시키기 위한 구현 팁을 정리한다. momentum, GPU minibatch inference, entropy augmentation, 출력 쪽 gradient만 계산하는 방식 등이 소개된다. 학습도 local energy 선학습, global energy 학습, 마지막 joint fine-tuning의 순서로 진행한다. 즉, SPEN의 성능은 모델 구조뿐 아니라 prediction과 training의 세부 최적화 전략에도 크게 의존한다.

#### A.5. Hyperparameters
이 절은 prediction과 training에 사용한 실제 설정값을 간단히 정리한다. prediction은 momentum 0.95, learning rate 0.1 기반의 gradient descent를 사용하고, training은 momentum 0.9의 SGD를 사용한다. 또한 $l_2$ regularization, 종료 조건, hidden size 선택 방식도 함께 언급된다. 부록의 dataset 요약표 역시 본문 실험 결과를 해석하기 위한 기본 맥락을 제공한다.
...
* 2026.03.26: [Final_ver] 파트 6-Appendix 글 작성
