...
기존의 로봇 제어 방식이 상위 레벨의 궤적 생성과 하위 레벨의 액추에이터 제어를 분리했던 것과 달리, 본 논문은 상태를 actuator command로 직접 mapping하는 방식을 통해 사전 정의된 제어 구조 없이도 복잡한 제어가 가능함을 보여준다.  

특히, 본 연구에서는 기존의 강화학습 기법들과 차별화된 새로운 Deterministic On-policy 학습 알고리즘을 제안한다.  
이 알고리즘은 복잡한 작업에 대해 보수적이지만 매우 안정적인 학습 성능을 보이며, 기존 알고리즘 대비 쿼드로터 제어에 더 적합한 특성을 가진다.  

저자들은 시뮬레이션뿐만 아니라 실제 쿼드로터 하드웨어를 통해 훈련된 정책의 성능을 입증하였다.  
실험 결과, 해당 정책 네트워크는 스텝 응답에 대해 상대적으로 정확하게 반응할 뿐만 아니라, 5 m/s의 초기 속도로 뒤집힌 채 공중에 던져지는 것과 같은 매우 가혹한 초기화 조건에서도 기체를 안정화할 수 있음을 보여준다.  
또한, 정책 평가에 소요되는 연산 시간은 타임 스텝당 $7\mu s$에 불과하여, 근사 모델을 사용하는 일반적인 궤적 최적화 알고리즘보다 두 자릿수 이상 빠른 연산 속도를 달성하였다.  
...
이에 따라 본 연구에서는 결정론적(Deterministic) 정책의 효율성과 On-policy 방법의 안정성을 결합한 새로운 학습 프레임워크를 제안한다. 이는 고차원 연속 행동 공간을 가진 로봇 제어 문제에 최적화된 접근 방식이다.  
